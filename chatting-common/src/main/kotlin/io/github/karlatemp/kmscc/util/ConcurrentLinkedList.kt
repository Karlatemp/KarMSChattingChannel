 ...
 * @create 2020/07/25 15:35:30
 *
 * KarMSChattingChannel/KarMSChattingChannel.chatting-common.main/ConcurrentLinkedList.kt
 */
...
class ConcurrentLinkedList<T> : Iterable<T> {
    ...
    fun insertFirst(value: T): LinkNode<T> =
        LinkNode(value).also { head.insertAfter(it) }


    fun insertLast(value: T): LinkNode<T> =
        LinkNode(value).also { tail.insertBefore(it) }
